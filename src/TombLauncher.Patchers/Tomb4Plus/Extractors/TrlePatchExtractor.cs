...
public class TrlePatchExtractor
{
    ...
    private FontInfo? ReadFontInfo(BinaryReader binaryReader)
    {
        var fontInfo = new FontInfo();

        const byte defaultCriticalBlinkInterval = 5;
        fontInfo.TextOrCriticalBarBlinkInterval =
            binaryReader.ReadByteAt(0x000521B0).NullIf(defaultCriticalBlinkInterval);

        fontInfo.MainFontMainColor =
            binaryReader.GetRgbColorAtAddress(0x000ADEF8).NullIf(FontInfo.DefaultFontMainColor);
        fontInfo.MainFontFadeColor =
            binaryReader.GetRgbColorAtAddress(0x000ADEFC).NullIf(FontInfo.DefaultFontMainColor);
        fontInfo.OptionsTitleFontMainColor =
            binaryReader.GetRgbColorAtAddress(0x000ADF18).NullIf(FontInfo.DefaultOptionsMainColor);
        fontInfo.OptionsTitleFontFadeColor =
            binaryReader.GetRgbColorAtAddress(0x000ADF1C).NullIf(FontInfo.DefaultOptionsFadeColor);
        fontInfo.InventoryTitleFontMainColor = binaryReader.GetRgbColorAtAddress(0x000ADF28)
            .NullIf(FontInfo.DefaultInventoryFontMainColor);
        fontInfo.InventoryTitleFontFadeColor = binaryReader.GetRgbColorAtAddress(0x000ADF2C)
            .NullIf(FontInfo.DefaultInventoryFontFadeColor);
        fontInfo.InventoryItemFontMainColor =
            binaryReader.GetRgbColorAtAddress(0x000ADF10).NullIf(FontInfo.DefaultFontMainColor);
        fontInfo.InventoryItemFontFadeColor = binaryReader.GetRgbColorAtAddress(0x000ADF14)
            .NullIf(FontInfo.DefaultInventoryItemFontFadeColor);
